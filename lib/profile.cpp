    ...
    while (true){
        ...
        switch (option){
            case 0:
                ...
                loading("Exiting PROFILE SELECTION");
                return 0;
            case 1:
            ...
            case 3:
                ...
                break;
            default:
                pause_invalid(); // ERROR HANDLING (Invalid user input)
                continue;
        }
    }
}
...
void add_profile(char profile_mode){
    ...
    if (search_profile(profile_name, profile_mode) == true){
        loading("File already exists. Returning to main menu");
        return;
    }
    ...
    // prompts user that the profile is being added
    loading("Adding profile " + profile_name);
...
int open_profile(char profile_mode, std::string* file_name){
    ...
    if (found){
        ...
        *file_name = directory + profile_name + FILE_EXTENSION;
        ...
        loading("Opening " + profile_name);

        // go signal to read profile file
        return 1;
    } else {
        loading("File not found. Returning to main menu"); // ERROR HANDLING (File is not found)
...
void delete_profile(char profile_mode){
    ...
    if (found){
        if ( !user_confirm("Are you sure you want to delete this profile?") ){
            loading("Profile deletion cancelled. Returning to profile selection menu");
            return;
        };
        ...
        loading("File not found. Returning to profile selection menu"); // ERROR HANDLING (File is not found)
    }
}
...
void display_profiles(char profile_mode){
    ...
    for ( const auto &entry: fs::directory_iterator(directory) ){
        ...
        // ignores files that arent .dat
        if (file_name.substr(file_name.length() - 4, 4) != FILE_EXTENSION) continue;

        // delete .dat extension
