...
int Account::select_record_screen(){
    ...
    while (page <= total_pages){  
        ...
        while (loop){
            ...
            std::cout << "Enter index of record to select (1 to 5)\n";
...
void Account::set_balance(double bal, bool transfer_call){
    ...
    if (diff > 0){
        ...
        loading("Balance didn't change. Returning to main menu");
        return;
    }
    ...
    option = user_confirm("Proceed with these changes?");

    if (option){
        add_to_records(record_diff);
        update_balance(record_diff);
    } else {
        loading("Modifying cancelled. Returning to main menu");
    }
}

void Account::transfer_balance(double amount, bool current){
    Record record_diff;
    ...
    record_diff.amount = amount;
    record_diff.category = "TRANSFERRED AMOUNT";
    record_diff.note = (current) ? "Amount transferred to" : "Amount transferred from";
...
void Account::modify_record(Record record, bool add_record = false){
    ...
    while (loop){
        ...
        switch(selection){
            ...
            case 1:
                while (loop2){
                    ...
                    if ( !date_is_valid(new_record.date) ){
                        pause_invalid("Invalid date. Please try again");
                        continue;
                    }
            ...
            // modify note
            case 5:
                std::cout << "\nEnter note (input NONE to leave as blank): ";
...
bool Account::filter_by_date(Date date, bool display_call, int* count){
    bool found = false;
